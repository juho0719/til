...
array([[1,2,3],
	   ...
	   [10,11,12]])
```
- `r_`와 같은 함수로 `concatenate((배열1,배열2), axis=0)`을 사용 가능. 단, 배열1, 배열2 차원은 같아야 함
```python
np.concatenate((a,b), axis=0)    # 0은 row를 의미
```
```
array([[1,2,3],
	   [4,5,6],
...
```
- `c_`와 같은 함수로 `concatenate((배열1,배열2), axis=1)`을 사용 가능. 단, 배열1, 배열2 차원은 같아야 함
```python
...
## 배열 연산
- 같은 인덱스의 성분끼리 덧셈,뺄셈,곱셈,나숫셈과 같은 연산 처리
```python
